...
const uploadRoot = path.resolve(process.cwd(), 'uploads', 'camera-reference-images');
...
export function registerCameraRoiRoutes(app: FastifyInstance) {
  ...
  app.get('/api/v1/cameras/:id/roi-reference-image', async (request, reply) => {
    ...
    if (!image?.storageKey) throw new HttpError(404, 'Reference image not found', 'REFERENCE_IMAGE_NOT_FOUND');
    const filePath = path.join(uploadRoot, image.storageKey);
    await stat(filePath);
    return reply.type(image.mimeType ?? 'image/jpeg').send(createReadStream(filePath));
  });
  ...
  async function saveReferenceImage(
    ...
    const storageKey = `${cameraId}/${imageId}.${ext}`;
    ...
    await prisma.camera.update({ where: { id: cameraId }, data: { analyticsConfiguration } });
    await prisma.auditLog.create({
      data: {
        userId: request.userContext?.id,
